...
class ImageTiler:
    ...
    def split_image_and_annotations(self, image, target, patchsize, show_patches=False):
        ...
        if target is not None:
            ...
            #area = target['area'] # recalculate area of segmented object mask
            ...
            new_patch_target_dict = {patch_bounds: [patch, 
                                                    {
                                                     'boxes': [], 
                                                     'labels': [], 
                                                     'image_id': [], 
                                                     'area': [],
                                                     'iscrowd': [],
                                                     'masks':[]}] for patch_bounds, patch in patch_target_dict.items()}
            for idx, (patch_bounds, patch) in enumerate(patch_target_dict.items()):
                ...
                if len(filtered_bboxes) == 0 or len(filtered_labels) == 0 or len(filtered_masks) == 0:
                    ...
                    # recalculate area of segmentation mask inside the patch using pycocotools
            
                    for filtered_mask in filtered_masks:
                        rle = cocomask.encode(np.asfortranarray(filtered_mask))
                        area.append(cocomask.area(rle))

                    iscrowd = np.zeros((len(filtered_masks),), dtype=np.int64)
                    ...
                    new_patch_target_dict[patch_bounds] = [patch, {'boxes': filtered_bboxes, 
                                                                   'labels': filtered_labels, 
                                                                   'image_id': image_id, 
                                                                    'area': area, 
                                                                    'iscrowd': iscrowd, 
                                                                    'masks': filtered_masks}]
                    #print(f"new_patch_target_dict[patch_bounds]: {new_patch_target_dict[patch_bounds]}")
    ...
    def prepare_patch_anno_dict_for_torch(self, patch_anno_dict):
        ...
        for patch_bounds, data in patch_anno_dict.items():
            ...
            if targets is None:
                ...
                area = np.array(targets['area'], dtype=np.float32)
                area = torch.as_tensor(area, dtype=torch.float32)
                iscrowd = torch.as_tensor(targets['iscrowd'], dtype=torch.int64)
   
                # update the patch_anno_dict
                patch_anno_dict[patch_bounds] = [patch, {'boxes': bboxes, 
                                                        'labels': labels, 
                                                        'image_id': image_id, 
                                                        'area': area, 
                                                        'iscrowd': iscrowd, 
    ...
    def process_image(self, original_img, target, show_patches=False):
        ...
        indices, patch_anno_dict = self.split_image_and_annotations(original_img, target=target, patchsize=self.patchsize, 
                                                                    show_patches=False)
