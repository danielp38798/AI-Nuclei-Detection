...
class Trainer:
    ...
    This class is used in the GUI for training the model.
    ...
    def __init__(self, config_dict):
        ...
        self.use_cuda = config_dict.get("use_cuda", True)
        self.num_classes = config_dict.get("num_classes", 2) + 1 # add background class
        self.lr = config_dict.get("lr", 0.001)
        self.num_epochs = config_dict.get("epochs", 50)
        self.batch_size = config_dict.get("batch_size", 2)
        self.backbone = None
        self.optimizer = None
        self.momentum = config_dict.get("momentum", 0.9)
        self.weight_decay = config_dict.get("weight_decay", 0.0005)
        self.root_dir = config_dict.get("data_dir", "data")
        self.checkpoints_dir = config_dict.get("checkpoints_dir", "training_checkpoints")
        print("Checkpoints dir: ", self.checkpoints_dir)
        self.do_evaluation_during_training = config_dict.get("do_evaluation_during_training", True)
        self.perform_slicing = config_dict.get("perform_slicing", True)
        self.print_freq = config_dict.get("print_freq", 400)
        self.eval_freq = config_dict.get("eval_freq", 1)
        self.do_model_evaluation = config_dict.get("do_model_evaluation", False)
        self.pretrained = config_dict.get("pretrained", True)
        self.trainable_layers = config_dict.get("trainable_layers", 3)
        self.backbone_name = config_dict.get("backbone", "resnet50")
        self.optimizer_name = config_dict.get("optimizer", "SGD")
    
        
        self.lr_scheduler_list = []
        self.early_stopping = EarlyStopping(patience=10, verbose=True, 
                                            path=os.path.join(self.checkpoints_dir, "early_stopping_checkpoint.pth"))
    ...
    def load_data(self):
        ...
        print("Checkpoint directory: {}".format(self.checkpoints_dir))
    ...
    def load_checkpoint(self):
        ...
        """
        ext = ".pth"
        prefix = os.path.join(self.checkpoints_dir, "checkpoint")
        ...
        final_model_path = os.path.join(self.checkpoints_dir, f'maskrcnn_model_final_{self.num_epochs}_epochs.pth')
    ...
    def save_checkpoint(self, epochs, **kwargs):
        ...
        ckpt_path = os.path.join(self.checkpoints_dir, file_name)
    ...
    def save_final_model(self):
        ...
        model_path = os.path.join(self.checkpoints_dir, f'maskrcnn_model_final_{self.num_epochs}_epochs.pth')
    ...
    def train(self):
        ...
        self.final_training_loss_dict = { i: {} for i in range(self.start_epoch, self.num_epochs)} 

        # keys are the epochs, values are the the dictionary with the average accuracies for each epoch over all batches
        self.final_validation_loss_dict = { i: {} for i in range(self.start_epoch, self.num_epochs)} 

        # keys are the epochs, values are the the dictionary with the average accuracies for each epoch over all batches
        self.final_accuracy_dict = { i: {} for i in range(self.start_epoch, self.num_epochs)} 

        # keys are epochs values are precision and recall
        self.precision_recall_dict = { i: {} for i in range(self.start_epoch, self.num_epochs)}


        self.hyperparams_dict = { i: {} for i in range(self.start_epoch, self.num_epochs)}

        # calculate the ETA for the training
        total_batches = len(self.train_data_loader)
        total_epochs = self.num_epochs
        total_iterations = total_batches * total_epochs
        print(f"Total number of batches: {total_batches}")
        print(f"Total number of epochs: {total_epochs}")
        print(f"Total number of iterations: {total_iterations}")


        for epoch in tqdm(range(self.start_epoch, self.num_epochs), desc="Training Progress", unit="epoch"):
            print("\n")
            ...
            if self.do_evaluation_during_training:
                ...
                if 'loss' in average_losses_dict.keys():
                    ...
                    self.early_stopping(val_loss, self.model, self.optimizer)
            ...
            # yield the ETA for the training after each epoch
            elapsed_time = time.time() - since
            elapsed_time = datetime.timedelta(seconds=elapsed_time)
            print(f"Elapsed time: {elapsed_time}")
            iterations_done = (epoch + 1) * total_batches
            iterations_left = total_iterations - iterations_done
            time_left = elapsed_time / iterations_done * iterations_left
