...
from gui.core.sahi.utils.compatibility import fix_full_shape_list, fix_shift_amount_list
...
class Yolov11DetectionModel(DetectionModel):
    ...
    def load_model(self):
        ...
        try:
            ...
            raise TypeError("model_path is not a valid yolov11 model path: ", e)

    def set_model(self, model: Any):
        """
        Sets the underlying Yolov11 model.
        Args:
            model: Any
                A Yolov11 model
        """
    ...
    def perform_inference(self, image: np.ndarray):
        ...
        if self.image_size is not None:  # ADDED IMAGE SIZE OPTION FOR YOLOV11 MODELS:
            prediction_result = self.model(
                image[:, :, ::-1], imgsz=self.image_size, verbose=False, device=self.device
            )  # YOLOV11 expects numpy arrays to have BGR
        else:
            prediction_result = self.model(
