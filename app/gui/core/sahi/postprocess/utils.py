...
def calculate_bbox_iob(pred1: ObjectPrediction, pred2: ObjectPrediction) -> float:
    """Returns the ratio of intersection area to the bigger box's area"""
    box1 = np.array(pred1.bbox.to_xyxy())
    box2 = np.array(pred2.bbox.to_xyxy())
    area1 = calculate_area(box1)
    area2 = calculate_area(box2)
    intersect = calculate_intersection_area(box1, box2)
    bigger_area = np.maximum(area1, area2)
    #return intersect / bigger_area
    # avoid division by zero
    if bigger_area == 0:
        return 0
    else:
...
def has_match(
    ...
    if match_type == "IOU":
        ...
        threshold_condition = calculate_bbox_iob(pred1, pred2) > match_threshold
    else:
        raise ValueError()
