...
from  gui.core.sahi.postprocess.utils import ObjectPredictionList, has_match, merge_object_prediction_pair
...
def nmm(
    ...
    for ind in range(len(object_predictions_as_tensor)):
        ...
        #else:
            #raise ValueError()
        elif match_metric == "IOB":
            # intersection over bigger area
            bigger = torch.max(rem_areas, areas[pred_ind])
            match_metric_value = inter / bigger
        else:
            raise ValueError()
