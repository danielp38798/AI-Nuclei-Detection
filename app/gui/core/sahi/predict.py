...
from  gui.core.sahi.utils.import_utils import is_available
...
from  gui.core.sahi.postprocess.combine import (
...
from  gui.core.sahi.prediction import ObjectPrediction, PredictionResult
from  gui.core.sahi.slicing import slice_image
from  gui.core.sahi.utils.coco import Coco, CocoImage
from  gui.core.sahi.utils.cv import (
    IMAGE_EXTENSIONS,
    VIDEO_EXTENSIONS,
...
from  gui.core.sahi.utils.file import Path, increment_path, list_files, save_json, save_pickle
from  gui.core.sahi.utils.import_utils import check_requirements
...
def get_sliced_prediction(
    ...
    perform_standard_pred: bool = False, # True
    ...
    #print(f"prior to postprocess: {len(object_prediction_list)}")

    # merge matching predictions
    if len(object_prediction_list) > 1:
        object_prediction_list = postprocess(object_prediction_list)
    
    #print(f"after postprocess: {len(object_prediction_list)}")
    ...
    #    if isinstance(object_prediction, ObjectPrediction):
    #        print(f"bbox [minx, miny, maxx, maxy]: [{object_prediction.bbox.minx}, {object_prediction.bbox.miny}, {object_prediction.bbox.maxx}, {object_prediction.bbox.maxy}]")
    #        print(f"score of object prediction: {object_prediction.score.value}")
