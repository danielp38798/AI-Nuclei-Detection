...
from gui.core.sahi.annotation import ObjectAnnotation
from gui.core.sahi.utils.coco import CocoAnnotation, CocoPrediction
from gui.core.sahi.utils.cv import read_image_as_pil, visualize_object_predictions
from gui.core.sahi.utils.file import Path
...
class PredictionResult:
    ...
    def export_visuals(
        ...
        self.results_dict = visualize_object_predictions(
    ...
    def store_binary_masks(self, export_dir, file_name="mask", format="png"):
        """
        Store binary masks of the predictions in the given directory.

        Args:
            export_dir: directory for resulting masks to be exported
            file_name: saving name
        """
        if not os.path.exists(export_dir):
            os.makedirs(export_dir)
        for idx, object_prediction in enumerate(self.object_prediction_list):
            image_filename = os.path.join(export_dir, f"{file_name}_{idx}.{format}")
            object_prediction.mask.export_bool_mask(path=image_filename)

    def to_coco_annotations(self, image_id):
        coco_annotation_list = []
        for object_prediction in self.object_prediction_list:
            coco_annotation_list.append(object_prediction.to_coco_prediction(image_id=image_id).json)
