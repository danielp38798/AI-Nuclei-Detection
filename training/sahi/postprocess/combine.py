...
def batched_nms(predictions: torch.tensor, match_metric: str = "IOU", match_threshold: float = 0.5):
    ...
    for category_id in torch.unique(category_ids):
        ...
        # print the dimensions of curr_indices and curr_keep_indices
        print(f"curr_indices: {curr_indices}")
        print(f"curr_keep_indices: {curr_keep_indices}")
        # Convert curr_keep_indices to a tensor
        curr_keep_indices = torch.tensor(curr_keep_indices, dtype=torch.long, device=curr_indices.device)

        # Ensure curr_keep_indices is not empty before indexing
        if curr_keep_indices.numel() > 0 and curr_indices.numel() > 0:

            # skip the case where curr_indices = tensor([0]) and curr_keep_indices = tensor([])
            if torch.sum(curr_indices) == 0:
                print(f"curr_indices is empty for category_id: {category_id}")
                print(f"curr_keep_indices: {curr_keep_indices}")
                continue
            keep_mask[curr_indices[curr_keep_indices]] = True
        # If curr_keep_indices is empty, we can skip the indexing
        # else:
        #     print(f"curr_keep_indices is empty for category_id: {category_id}")
        #     print(f"curr_indices: {curr_indices}")
        #     print(f"keep_mask: {keep_mask}")
    # print the dimensions of keep_mask
        #keep_mask[curr_indices[curr_keep_indices]] = True
    keep_indices = torch.where(keep_mask)[0]

    # print the dimensions of keep_indices
    # If no indices are selected, return an empty list
    # print the dimensions of keep_indices
...
class NMMPostprocess(PostprocessPredictions):
    def __call__(
        ...
        object_prediction_list = ObjectPredictionList(object_predictions)
        print(f"object_prediction_list: {object_prediction_list}")
        object_predictions_as_torch = object_prediction_list.totensor()
        if self.class_agnostic:
