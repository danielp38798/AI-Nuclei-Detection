    ...
    def merge_datasets(self):
        ...
        if len(imports_with_annotations) == 1:
            ...
            for idx, (key, value) in enumerate(imports_with_annotations.items()):
                ...
                annotations_path = os.path.join(value['annotations'], 'instances_default.json')
                images_path = value['images']
                #annotations_path = os.path.join(folder, 'annotations_coco', 'instances_default.json')
            ...
            output_annotations_path = os.path.join(self.merged_data_dir, 'annotations_coco', 'instances_default.json')
            # create output file
...
class MergeDataDialogManual(QtWidgets.QDialog):
    ...
    def merge_data(self):
        ...
        for folder in self.input_folders:
            if not os.path.exists(os.path.join(folder, 'annotations_coco', 'instances_default.json')):
    ...
    def merge_datasets(self, input_folders, output_folder):
        ...
        # Save the merged annotations to the output folder
        output_annotations_path = os.path.join(output_folder, 'annotations_coco', 'instances_default.json')
        # create output file
        if not os.path.exists(os.path.dirname(output_annotations_path)):
            os.makedirs(os.path.dirname(output_annotations_path))
        with open(output_annotations_path, 'w') as output_file:
...
class MainWindow(QWidget):
    ...
    def init_ui(self):
        ...
        button1 = QPushButton('Image Import', self)
        self.import_window_opened = 0
        button1.clicked.connect(self.open_window1)
        layout.addWidget(button1)

        button2 = QPushButton('Nuclei Pre-Annotation', self)
        button2.clicked.connect(self.open_window2)
        layout.addWidget(button2)

        button3 = QPushButton('COCO Dataset Import', self)
        button3.clicked.connect(self.open_window3)
        layout.addWidget(button3)

        button4 = QPushButton('Dataset Merging', self)
        button4.clicked.connect(self.open_window4)
        layout.addWidget(button4)

        button5 = QPushButton('Image Converter', self)
    ...
    def open_window4(self):
        self.check_for_annotated_data()
        if len(self.annotated_data) > 0:
            # ask the user whether they want to all created data or selected data
            merge_data_question = QtWidgets.QMessageBox(self)
            merge_data_question.setIcon(QtWidgets.QMessageBox.Question)
            merge_data_question.setWindowTitle("Merge Data")
            merge_data_question.setText("Annotations detected. \n Do you want to merge all data or selected data folders?")

            # Add custom buttons with text
            merge_data_question.addButton("Merge All", QtWidgets.QMessageBox.YesRole)
            merge_data_question.addButton("Merge Selected", QtWidgets.QMessageBox.NoRole)
            result = merge_data_question.exec_()
            if result == 0:
                # merge all data
                self.merge_dialog = MergeDataDialog(imports_dir=self.imports_dir, output_dir=self.output_dir)
                self.merge_dialog.exec_()
                if not self.merge_dialog.stop_process:
                    self.current_state_label.setText(f"Current State: Dataset merging finished")
            else:
                # merge selected data
                self.merge_dialog = MergeDataDialogManual(imports_dir=self.imports_dir, output_dir=self.output_dir)
                self.merge_dialog.exec_()
                if not self.merge_dialog.stop_process:
                    self.current_state_label.setText(f"Current State: Dataset merging finished")
        else:
            # show an info box that no data was found
            QtWidgets.QMessageBox.information(self, "Info", "No annotated data found. You may merge data manually.")
    ...
    def update_paths(self):
        ...
        self.annotations_output_dir = os.path.join(self.run_subdir, "annotations_coco")
