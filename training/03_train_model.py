#!/usr/bin/env python
"""
Mask R-CNN train script based on the scripts provided in detectron2 git repository: https://github.com/facebookresearch/detectron2
Running this script will perform the training of the Detectron2 model on the custom dataset. 
It includes the setup of the configuration, registration of the dataset, and definition of the training loop with evaluation hooks.
"""
...
#from tools.copy_paste import CopyPaste, CocoDetectionCP
...
class LossEvalHook(HookBase):
    ...
    def after_step(self) -> None:
        ...
        if self.trainer.iter == 0 or is_final or (self._period > 0 and next_iter % self._period == 0):
            self._do_loss_eval()
    ...
    def check_if_early_stop(self) -> None:
        ...
        validation_losses = self.trainer.storage.history('val_loss').values()
...
class Trainer(DefaultTrainer):
    ...
    @classmethod
    def test_with_trainset(cls, cfg, model, evaluators=None):
        """
        ...
        # add "_train" suffix to the keys of the results
        results = OrderedDict(
            (k + "_train", v) for k, v in results.items()
        )
        if comm.is_main_process():
    ...
    @classmethod
    def perform_eval(cls, cfg, model) -> OrderedDict:
        """
        Perform evaluation on the validation set and the training set.
        ...
        Returns:
            OrderedDict: Evaluation results for both validation and training sets.
        """
        logger = logging.getLogger(__name__)
        evaluators = [
            cls.build_evaluator(
        ...
        # will perform the testing on the test set
        res = cls.test(cfg, model, evaluators)
        # add "_val" suffix to the keys of the results
        res = OrderedDict(
            (k + "_val", v) for k, v in res.items()
        )
        if comm.is_main_process():
            verify_results(cfg, res)
        
        #perform testing on the train set
        res_train = cls.test_with_trainset(cfg, model)
    ...
    def build_hooks(self) -> list:
        ...
        #ret.append(hooks.EvalHook(cfg.TEST.EVAL_PERIOD, test_on_val_set))
        
      
        if cfg.TEST.AUG.ENABLED:
            print("Using TTA for evaluation")
            ret.append(hooks.EvalHook(cfg.TEST.EVAL_PERIOD, lambda: Trainer.test_with_TTA(cfg, self.model)))
            ret.append(hooks.BestCheckpointer(eval_period=cfg.TEST.EVAL_PERIOD, 
                                        checkpointer=self.checkpointer,
                                        val_metric="segm_val/AP50"))
            ...
            print("Not using TTA for evaluation")
            #test on val set
            #ret.append(hooks.EvalHook(cfg.TEST.EVAL_PERIOD, test_on_val_set))
            
            # test on both train and val set in one eval hook
            ...
            if comm.is_main_process():
                ...
                val_test_period = cfg.TEST.EVAL_PERIOD
                ret.append(LossEvalHook(val_test_period, self.model, 
                                        build_detection_test_loader(self.cfg, self.cfg.DATASETS.TEST[0], DatasetMapper(self.cfg, True))))
            
                ret.append(hooks.EvalHook(cfg.TEST.EVAL_PERIOD, lambda: Trainer.perform_eval(cfg, self.model)))
                

                ret.append(hooks.BestCheckpointer(eval_period=cfg.TEST.EVAL_PERIOD, 
                                            checkpointer=self.checkpointer,
                                            val_metric="segm_val/AP50",
                                            file_prefix = "best_checkpoint_segm")) 

                ret.append(hooks.BestCheckpointer(eval_period=cfg.TEST.EVAL_PERIOD, 
                                            checkpointer=self.checkpointer,
                                            val_metric="bbox_val/AP50",
                                            file_prefix = "best_checkpoint_bbox")) 
...
def setup(args) -> get_cfg:
    ...
    if args is not None and args.config_file:
        print("Config file given in args:", args.config_file)
        cfg.merge_from_file(args.config_file)
    else:
        yaml_path = f"./configs/COCO-InstanceSegmentation/{config}"
        cfg.merge_from_file(yaml_path)
        print("No config file given in args. Using default config file:", yaml_path)

    # Check if options are given in args
    if args is not None and args.opts:
        print("Options given in args:", args.opts)
        cfg.merge_from_list(args.opts)

    # Register the custom COCO dataset
    register_coco_instances("my_dataset_train2017", {}, 
                            r"./TRAINDATA/sliced_coco/annotations/instances_train2017.json", 
                            r"./TRAINDATA/sliced_coco/train2017")
    ...
    cfg.MODEL.WEIGHTS = model_zoo.get_checkpoint_url(f"COCO-InstanceSegmentation/{config}")
    ...
    batch_size = 1 # 2  # Replace with your actual batch size
    num_epochs = 25  # Replace with your desired number of epochs

    # Calculate max_iter
    iter_per_epoch = num_train_images // batch_size
    print(f"Number of iterations per epoch: {iter_per_epoch}")
    ...
    cfg.OUTPUT_DIR = f'./training_output_{time.strftime("%Y-%m-%d-%H-%M-%S")}'
...
def main(args: Namespace) -> Optional[OrderedDict]:
    ...
    if args is not None and args.eval_only:
...
def run_trainer():
    """
    Run the trainer by invoking the main function with command-line arguments.

    This function is a wrapper around the `invoke_main` function to provide a cleaner interface
