            ...
            for epoch in data:
                ...
                if data[epoch] != {}:
                    for iou_type in data[epoch]:
                        if iou_type == 'bbox':
                            AP_50 = data[epoch][iou_type]['0.5'].get(str(category), {}).get('AP_50', 0)
                            #AP_50_bbox.append(data[epoch][iou_type]['0.5'][str(category)]['AP_50'])
                            AP_50_bbox.append(AP_50)
                            #f1_score_bbox.append(data[epoch][iou_type]['0.5'][str(category)]['f1_score'])
                            f1_score = data[epoch][iou_type]['0.5'].get(str(category), {}).get('f1_score', 0)
                            f1_score_bbox.append(f1_score)
                            ...
                            AP_50 = data[epoch][iou_type]['0.5'].get(str(category), {}).get('AP_50', 0)
                            AP_50_segm.append(AP_50)
                            f1_score = data[epoch][iou_type]['0.5'].get(str(category), {}).get('f1_score', 0)
                            f1_score_segm.append(f1_score)
